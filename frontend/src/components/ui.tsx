...
export function Icon({ name, size = 14, color = "currentColor" }: IconProps): JSX.Element | null {
  ...
  switch (name) {
    ...
    case "archive":
      return (
        <svg width={size} height={size} viewBox={v}>
          <path d="M2 4H14V6H2V4ZM3 6V13H13V6M6 9H10" {...stroke} />
        </svg>
      );
    case "sun":
      return (
...
/**
 * Inline tag rendered next to a chat answer that was generated by an
 * external provider, e.g. "via Anthropic claude-sonnet-4-6". Uses the
 * generic ``ext`` glyph plus mono font for the model id so the
 * provider/model pairing is unambiguous.
 */
export function ExternalCallTag({
  provider,
  model,
}: {
  provider: string;
  ...
  return (
    <span
      className="t-meta t-12"
      style={{ display: "inline-flex", alignItems: "center", gap: 6 }}
    >
      <Icon name="ext" size={12} />
      via {provider}
      {model && <span className="t-mono">{model}</span>}
    </span>
  );
}
